...
client.aqua.on(AqualinkEvents.TrackError, (player, track, payload) => {
    ...
    const channel = client.channels.cache.get(player.textChannel);
    if (channel) channel.send('❌ An error occurred while playing the track!');
...
client.aqua.on(AqualinkEvents.TrackStuck, (player, error, track) => {
    ...
    const channel = client.channels.cache.get(player.textChannel);
    if (channel) channel.send('⚠️ Track appears to be stuck, skipping...');
    player.skip();
});
```
...
client.aqua.on(AqualinkEvents.LyricsNotFound, (player, track, payload) => {
    ...
    const channel = client.channels.cache.get(player.textChannel);
    if (channel) channel.send('📝 No lyrics available for this track.');
});
```
